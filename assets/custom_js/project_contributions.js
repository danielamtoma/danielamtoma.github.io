...
function projectContribution(){
    ...
    $.getJSON('data/projects/'+id+'.json', function(description){
      $("#myProjectGraph_3_description").html(
        ...
        '<br/><b>Coût total du projet:</b> ' + description.totalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'") + ' €' +
        '<br/><b>Contribution EU:</b> ' + description.ecMaxContribution.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'") + ' €' +
        ''
      );
    });
    ...
    $.getJSON('data/projects/'+id+'.json', function(jsondata){
      var dataset = [];
      for (var element in jsondata.projectParticipants) {
        var projectcontribution = jsondata.projectParticipants[element].ecContrib;
        var projectname = jsondata.projectParticipants[element].orgName;
        var projectrcn = jsondata.projectParticipants[element].orgId;
      ...
      var vis = d3.select("#myProjectGraph_3_1").append("svg:svg")
        .attr("width", w)
        .attr("height", h);
        ...
        function filterData(element, index, array) {
          ...
          valueLabels.enter().append("svg:text")
            ...
            .attr("text-anchor", function(d){
              ...
              var percentage = (d.value/sliceProportion)*100;
              return d.name.substring(0,10)+"...: "+percentage.toFixed(1)+"%";
            })
            .on("click", function(d){
              $("#companyId").html(d.id);
              companyId(d.name);
